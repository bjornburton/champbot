...
\Y\B\&{inputStruct} \\{input\_s} $\K$ $\{$ $.$ $\\{edge}\K\.{CH2RISE}$ $\}$  ;\7
...
$.$ $\\{minOut}\K\T{1}$ $,$ $.$ $\\{maxOut}\K\T{255}$ $\}$  ;\7
\X30:Initialize the inputs and capture mode\X\X27:Initialize pin outputs\X\Y\par
\fi
...
\Y\B\X28:Configure to idle on sleep\X\7
\\{ledcntl}(\.{OFF});\6
