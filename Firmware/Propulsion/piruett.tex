...
\X32:Initialize the inputs and capture mode\X\X29:Initialize pin outputs\X\Y\par
\fi
...
\Y\B\X30:Configure to idle on sleep\X\7
\\{ledcntl}(\.{OFF});\6
...
\M{18}
Now that a loop is started, we wait in \PB{\.{"idle"}} for the edge on the
channel
selected.
...
\Y\B\&{if} ${}(\\{translation\_s}.\\{radius}\G\T{255}){}$\1\5
\\{ledcntl}(\.{ON});\2\6
\&{else}\1\5
\\{ledcntl}(\.{OFF});\2\7
$\}{}$\SHC{ end for }\7
...
\N{1}{28}These are the supporting routines, procedures and configuration
blocks.
...
\M{29}\B\X29:Initialize pin outputs\X${}\E{}$\6
${}\{{}$\C{ set the led port direction; This is pin \#17 }\1\6
${}\.{DDRB}\MRL{{\OR}{\K}}(\T{1}\LL\.{DDB5});{}$\6
\4${}\}{}$\2\par
\U14.\fi

\M{30}\B\X30:Configure to idle on sleep\X${}\E{}$\6
${}\{{}$\1\6
...
\M{32}\B\X32:Initialize the inputs and capture mode\X${}\E{}$\6
...
\\{uint16\_t}\\{input}){}$\1\1 $\{{}$\7
\&{const} \\{int32\_t}\\{ampFact}${}\K\T{128\$L}{}$;\SHC{ factor for precision
}\par
\fi
...
extra
bits for precision.
\Y\B$\\{int32\_t}\\{gain}\K(\\{ampFact}*(\\{int32\_t})(\\{inputScale\_s}\MG%
...
carve a
\PB{\.{"turn"}}. This procedure should do this but it's not going to be
perfect.
