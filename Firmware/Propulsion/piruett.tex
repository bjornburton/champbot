...
\M{7}Here is a structure to keep track of the state of intput things,
like servo timing.
...
\\{ch1fall}\K\T{0}$ $,$ $.$ $\\{ch2fall}\K\T{0}$ $,$ $.$ $\\{edge}\K\T{0}$ $\}$
 ; \X26:Initialize the inputs and capture mode\X\X23:Initialize pin outputs\X%
\par
\fi
...
\Y\B\X24:Configure to idle on sleep\X\\{ledcntl}(\.{OFF});\par
\fi
...
\N{1}{22}These are the supporting routines, procedures and configuration
blocks.
...
\M{23}\B\X23:Initialize pin outputs\X${}\E{}$\6
${}\{{}$\C{ set the led port direction; This is pin \#17 }\1\6
${}\.{DDRB}\MRL{{\OR}{\K}}(\T{1}\LL\.{DDB5});{}$\6
\4${}\}{}$\2\par
\U10.\fi

\M{24}\B\X24:Configure to idle on sleep\X${}\E{}$\6
${}\{{}$\1\6
...
\M{26}\B\X26:Initialize the inputs and capture mode\X${}\E{}$\6
